...
if (isset($_POST['agregar_prod'])) {
    ...
    $sql_prod = "INSERT INTO Producto (Codigo_Barras, precio, cantidad, nombre, Id_Proveedor) VALUES ('$cod', $precio, $cant, '$nom', '$prov')";
    
    if ($conn->query($sql_prod) === TRUE) {
        if ($tipo == 'duradero') {
            $sql_hija = "INSERT INTO Producto_Duradero (codigo_de_barras, caducidad, temperatura) VALUES ('$cod', '{$_POST['caducidad']}', {$_POST['temperatura']})";
        } else {
            $sql_hija = "INSERT INTO Producto_No_Duradero (codigo_de_barras, garantia) VALUES ('$cod', '{$_POST['garantia']}')";
        }

        if ($conn->query($sql_hija) === TRUE) {
            $conn->query("COMMIT");
